...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: props.student?.name || 'Student Details',
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-foreground">{{ student.name || 'Student Details' }}</h1>
                ...
                <div class="lg:col-span-1">
                    <div class="rounded-lg border bg-card p-6">
                        <div class="text-center">
                            ...
                            <h2 class="text-xl font-semibold text-foreground">{{ student.name || 'N/A' }}</h2>
                            <p class="text-sm text-muted-foreground">{{ student.student_id || 'N/A' }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="rounded-lg border bg-card p-6">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-foreground">{{ student.name || 'N/A' }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Student ID</dt>
                                <dd class="mt-1 text-sm text-foreground">{{ student.student_id || 'N/A' }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Class</dt>
                                <dd class="mt-1 text-sm text-foreground">{{ student.class || 'N/A' }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Section</dt>
                                <dd class="mt-1 text-sm text-foreground">{{ student.section || 'N/A' }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Created At</dt>
                                <dd class="mt-1 text-sm text-foreground">{{ student.created_at ? formatDate(student.created_at) : 'N/A' }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Last Updated</dt>
                                <dd class="mt-1 text-sm text-foreground">{{ student.updated_at ? formatDate(student.updated_at) : 'N/A' }}</dd>
                            </div>
                        </div>
                    </div>
                </div>
